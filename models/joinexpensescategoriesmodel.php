...
class JoinExpensesCategoriesModel extends Model{
    ...
    public function getAll($userId){
        $items=[];
        try {
            $query = $this->prepare('SELECT expenses.id as expense_id, title, category_id, amount, date, id_user, categories.id, name, color, FROM expense INNER JOIN categories where expenses.category_id = categories.id AND expenses.id_user = :userid ORDER BY date');
